                ...
                <div class="rounded-lg border bg-card text-card-foreground shadow-sm card-hover-glow">
                    <div class="p-6 pt-6 text-center">
                        <div class="w-14 h-14 rounded-lg gradient-bg flex items-center justify-center mx-auto mb-4">
                            <i class="<?= htmlspecialchars($solution['icon']) ?> text-white text-2xl"></i>
                        </div>
                        <h3 class="text-lg font-semibold mb-3"><?= htmlspecialchars($solution['title']) ?></h3>
                        <p class="text-sm text-muted-foreground">
                            <?= htmlspecialchars($solution['description']) ?>
                        </p>
                    </div>
                </div>
                <?php endforeach; ?>
        ...
        <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            <?php if(!empty($use_cases)): ?>
                ...
                <div class="rounded-lg border bg-card text-card-foreground shadow-sm gradient-border-card">
                    <div class="p-6 pt-6">
                        <h3 class="text-xl font-semibold mb-3"><?= htmlspecialchars($useCase['title']) ?></h3>
                        <p class="text-muted-foreground">
                            <?= htmlspecialchars($useCase['description']) ?>
                        </p>
                    </div>
                </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="col-span-3 text-center py-8">
                    <p class="text-muted-foreground">No use cases data available.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="container mx-auto max-w-4xl">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            <?php if(!empty($metrics)): ?>
                ...
                <div>
                    <div class="text-4xl font-bold gradient-text mb-2"><?= htmlspecialchars($metric['value']) ?></div>
                    <div class="text-sm text-muted-foreground"><?= htmlspecialchars($metric['label']) ?></div>
                </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="col-span-4 text-center py-8">
